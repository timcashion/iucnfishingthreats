...
author: "Tim Cashion^1^, Travis C. Tai^1^, Vicky W.Y. Lam^2^, Daniel Pauly^3^, and U. Rashid Sumaila^4^"
date: "`r date()`"
output: 
...
## Conclusion
Our results highlight areas of high conservation concern for particular fishing gears, and areas of high overlap with multiple fishing gear threats and multiple species of conservation concern. We also highlight areas with the potential for low-cost fishing closures leading to maximum protection of species negatively affected by these fishing gears. Interestingly, the study suggests that gillnet fisheries represent a greater overall threat than the often criticised bottom trawl fisheries due to the high fisheries profits derived from many bottom trawl catches. This analysis can help inform future conservation planning with areas of low-cost trade-offs in comparison to areas with much higher costs for equal conservation benefits. 

## Acknowledgements
TC acknowledges funding support of the Social Sciences and Humanities Research Council of Canada and the Four Year Fellowship from the University of British Columbia. DP acknowledges support from the *Sea Around Us*, which is funded by a number of philanthropic foundations. URS acknowlegdes funding support for the OceanCanada funded by the Social Sciences and Humanities Research Council of Canada, and additional funding from the ADM Capital Foundation.
...
eez_ids <- read.csv(file.path(dir_spatial_csvs, "eez_ids.csv")) %>%
  rename(eez_id = EEZID, eez = Name, entity=Entity) %>%
  dplyr::select(eez_id, eez, entity)
cell_ids <- left_join(cell_ids, eez_ids, by ="eez_id")
# 
# eez_ids <- eez_ids %>% 
#   mutate(eez = gsub(eez, pattern="ô", replacement="o"),
#          entity = gsub(entity, pattern="ô", replacement="o"),
...
gear_search_table[] <- lapply(gear_search_table, function(x) gsub(x, pattern="|", replacment="; "))
