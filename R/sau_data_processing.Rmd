...
non_min_min <- function(x=NA, na.rm=TRUE){
  #Returns value closest to min value that isn't the min if there is more than two values
  if(length(na.omit(x))==0){
    return(min(na.omit(x), 0))
  } else if(length(unique(x))>2){
    min_value <- min(x)
    x <- x[x!=min_value]
    return(min(na.omit((x))))
  } else {
    return(min(na.omit(x)))
  }
}
...
cell_id_area <- fasterize::fasterize(eez_data, cell_ids_rast, field = 'Water_Area', fun='sum') 
cell_id_area <- rasterize(eez_data, cell_ids_rast, field = 'Water_Area', fun=function(x, na.rm) sum(na.omit(x)))
...
write.csv(cell_id_area_df, file.path(dir_spatial, paste("cell_ids_area_", cell_size, ".csv", sep="")))
